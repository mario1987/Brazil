...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBrazilAmount"))
        settings.setValue("nAnonymizeBrazilAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBrazilAmount = settings.value("nAnonymizeBrazilAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBrazilAmount"))
        SoftSetArg("-anonymizeBrazilamount", settings.value("nAnonymizeBrazilAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBrazilAmount:
            nAnonymizeBrazilAmount = value.toInt();
            settings.setValue("nAnonymizeBrazilAmount", nAnonymizeBrazilAmount);
            emit anonymizeBrazilAmountChanged(nAnonymizeBrazilAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
