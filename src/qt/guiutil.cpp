...
{
    // Convert brazil:// to brazil:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("brazil://"))
    {
        uri.replace(0, 12, "brazil:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Brazil.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IBrazilLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IBrazilLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "brazil.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Brazil-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  brazil-qt [" + tr("command-line options") + "]                     " + "\n";
