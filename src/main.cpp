...
unsigned int nStakeMinAge = 2 * 60 * 60; // 2 hour's
...
const string strMessageMagic = "Brazil Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	{
		nSubsidy = 200000000 * COIN; // For Beta Testing & Master Nodes
	}
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;  // 10% yearly interest 
    }
    else if(pindexBest->nHeight > 100001)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) / 10;  // 1% yearly interest 
    }
