...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #brazilcoinTEST\r");
            Send(hSocket, "WHO #brazilcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #brazilcoin\n");
            // randomly join #brazilcoin00-#brazilcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #brazilcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #brazilcoin%02d\r", channel_number).c_str());
        };
